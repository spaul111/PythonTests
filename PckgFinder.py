...
# Function to find package-lock.json files and extract dependencies
def find_dependencies(repo_dir):
    dependencies = []
    for root, dirs, files in os.walk(repo_dir):
        for file in files:
            if file.lower() == 'package-lock.json':
...
def main():
    ...
    if clone_repository(repo_url, destination):
        ...
        if dependencies:
            # Warite dependencies to package-lock.json file
            with open('package-lock.json', 'w') as f:
                for dependency in dependencies:
                    f.write(dependency + '\n')
            print("Dependencies saved to package-lock.json file.")
        else:
